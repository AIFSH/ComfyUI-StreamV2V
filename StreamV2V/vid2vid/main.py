...
def main(
    ...
    output_file: str = os.path.join(CURRENT_DIR, "outputs"),
    ...
    # Specify LORAs
    # ComfyUI/custom_nodes/ComfyUI-StreamV2V/lora_weights
    lora_path = os.path.join(os.path.dirname(os.path.dirname(output_file)),"custom_nodes","ComfyUI-StreamV2V","lora_weights")
    if face_lora != "none":
        face_lora_path = os.path.join(lora_path,"face",face_lora)
        print(f"Use face LORA: in {face_lora_path}")
        stream.stream.load_lora(face_lora_path, adapter_name='face')
        stream.stream.pipe.set_adapters(["lcm", "face"], adapter_weights=[1.0, face_lora_weights])
    if style_lora != "none":
        style_lora_path = os.path.join(lora_path,"style",style_lora)
        print(f"Use style LORA:  in {style_lora_path}")
        stream.stream.load_lora(style_lora_path, adapter_name='style')
        stream.stream.pipe.set_adapters(["lcm", "style"], adapter_weights=[1.0, style_lora_weights])
    ...
    '''
    write_video(output_file, video_result, fps=fps)
